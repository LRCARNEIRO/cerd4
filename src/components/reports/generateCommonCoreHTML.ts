...
const STYLES = `
@page { size: A4; margin: 2.5cm; @bottom-center { content: counter(page); font-size: 9pt; color: #64748b; } }
...
.chart-inline { margin: 0.5cm 0; page-break-inside: avoid; }
...
export function generateCommonCoreHTML(
  ...
  const saudeSerieHistorica = mirror?.saudeSerieHistorica?.length ? mirror.saudeSerieHistorica : hcSaude;
...
<body>
  ...
  <div class="section">
    ...
    <table>
      <thead><tr><th>Documento (cadastrado na base)</th><th>Categoria</th><th>Fonte</th></tr></thead>
      <tbody>
        ${normativos.slice(0, 20).map((n: any) => `
          <tr>
            <td>${escapeHtml(n.titulo)}</td>
            <td>${escapeHtml(n.categoria || '—')}</td>
            <td>${n.url_origem ? `<a href="${escapeHtml(n.url_origem)}" target="_blank">link</a>` : '<em>sem URL</em>'}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
    <p style="font-size:8pt;color:#64748b;">Total de marcos normativos cadastrados: <strong>${normativos.length}</strong>. Fonte: <code>documentos_normativos</code>.</p>
    ` : `<p style="font-size:9pt;color:#64748b;border-left:3px solid #cbd5e1;padding-left:12px;"><em>Nenhum documento normativo cadastrado na base do sistema. A tabela de marcos legislativos será preenchida automaticamente quando documentos forem cadastrados em <code>documentos_normativos</code>.</em></p>`}
  </div>

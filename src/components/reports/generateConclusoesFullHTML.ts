...
export function generateConclusoesFullHTML(params: GenerateParams): string {
  ...
  const badgeHTML = (tipo: string) => {
    const map: Record<string, [string, string]> = {
      avanco: ['badge-success', '✓ Avanço'],
      retrocesso: ['badge-destructive', '✗ Retrocesso'],
      lacuna_persistente: ['badge-warning', '⚠ Lacuna'],
      paradoxo: ['badge-purple', '◈ Paradoxo'],
      correlacao: ['badge-info', '↔ Correlação'],
      tendencia: ['badge-info', '→ Tendência'],
      lacuna_critica: ['badge-destructive', '⚠ Lacuna Crítica'],
      alerta: ['badge-destructive', '⚠ Alerta'],
      progresso: ['badge-success', '✓ Progresso'],
      'contradição': ['badge-purple', '◈ Contradição'],
...
<div class="header">
  ...
  <p>Consolidação: Infográficos, Síntese, Fios Condutores, Cruzamentos, Lacunas, Avanços, Retrocessos e Aderência ICERD</p>
  <p>Gerado em: ${now}</p>
</div>
...
<div class="grid-2">
  ...
  <div class="avanco-box">
    ...
    <ul style="margin:0;padding-left:16px;font-size:10px;">
      ...
      <li>Analfabetismo negro: ${edu2018.analfabetismoNegro}% → ${edu2024.analfabetismoNegro}%</li>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Mortalidade materna negra (/100mil NV)</td><td>${sau2018.mortalidadeMaternaNegra}</td><td>${sau2024.mortalidadeMaternaNegra}</td><td>${(sau2024.mortalidadeMaternaNegra-sau2018.mortalidadeMaternaNegra).toFixed(1)}</td><td><span class="badge badge-warning">Parcial</span></td></tr>
...
<p class="source">Argumentos transversais: Base Estatística × ${totalLacunas} lacunas ONU × ${respostas.length} respostas CERD III × ${orcStats?.totalRegistros || 0} registros orçamentários.</p>
${fiosCondutores.map(fio => `
<div class="card ${cardClass(fio.tipo)}">
  <h4>${fio.titulo} ${badgeHTML(fio.tipo)}</h4>
  <p style="font-size:10px;">${fio.argumento}</p>
  ${fio.evidencias.length > 0 ? `<p class="source"><strong>Evidências:</strong> ${fio.evidencias.map(e => `${e.texto} (${e.fonte}${e.variacao ? ` | ${e.variacao}` : ''})`).join('; ')}</p>` : ''}
  ${fio.comparativo2018 ? `<p style="font-size:9px;background:#f1f5f9;padding:4px 8px;border-radius:4px;">📊 ${fio.comparativo2018}</p>` : ''}
</div>`).join('')}

<!-- 5. CRUZAMENTOS -->
<h2 id="sec5">5. Cruzamentos Analíticos (${insightsCruzamento.length})</h2>
${insightsCruzamento.map(ins => `
<div class="card ${cardClass(ins.tipo)}">
  <h4>${ins.titulo} ${badgeHTML(ins.tipo)}</h4>
  <p style="font-size:10px;">${ins.descricao}</p>
  ${ins.dados.length > 0 ? `<ul style="margin:4px 0;padding-left:16px;font-size:9px;">${ins.dados.map(d => `<li>${d}</li>`).join('')}</ul>` : ''}
</div>`).join('')}
...
<div class="card card-lacuna">
  ...
  ${c.evidencias.length > 0 ? `<p class="source">${c.evidencias.join('; ')}</p>` : ''}
</div>`).join('')}
...
<table>
  <tr><th>Artigo</th><th>Tema</th><th>Lacunas</th><th>Cumpr.</th><th>Parcial</th><th>Não Cumpr.</th><th>Score</th></tr>
  ${icerdData.map(a => `<tr>
    <td><strong>${a.numero}</strong></td>
    <td>${a.titulo}</td>
    <td>${a.total}</td>
    <td class="stat-pos">${a.cumpr}</td>
    <td>${a.parc}</td>
    <td class="stat-neg">${a.nao + a.retro}</td>
    <td><strong>${a.score}%</strong></td>
  </tr>`).join('')}
</table>
${icerdData.map(a => `
<div style="margin:8px 0;">
  <div style="font-size:10px;font-weight:600;">${a.numero} — ${a.titulo} (${a.score}%)</div>
  <div class="aderencia-bar"><div class="aderencia-fill" style="width:${a.score}%;background:${barColor(a.score)}">${a.score}%</div></div>
</div>`).join('')}
...
<div class="highlight">
  <p style="font-weight:700;margin-bottom:6px;">Avaliação Geral</p>
  <p>O Estado brasileiro apresenta um padrão de <strong>paradoxo normativo-implementação</strong>: avanços legislativos significativos 
  ...
  indicando cumprimento parcial das obrigações convencionais.</p>
</div>
